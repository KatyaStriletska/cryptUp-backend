...
async fn get_ba_keypair() -> Result<Keypair, String> {
    dotenv().ok();
    println!("get_ba_keypair!!!!!");
    // let rpc_url = "https://api.devnet.solana.com"; // або mainnet
    ...
    let keypair = Keypair::from_bytes(&private_key_vec).expect("Invalid Keypair");
...
fn get_rpc_client() -> Result<RpcClient, String> {
    dotenv().ok();
    let rpc_url = std::env::var("DEFAULT_RPC_CLIENT")
    .unwrap_or_else(|_| "https://api.devnet.solana.com".into());
    println!("Connecting to RPC at: {}", rpc_url);
    Ok(RpcClient::new(rpc_url.to_string()))
}
async fn create_base_multisig(create_key: &Keypair) -> Result<BaseMultisig, String> {
    ...
    match rpc_client.get_balance(&creator_keypair.pubkey()).await {
